...
public final class FermiLiteAssemblyHandler implements FindBreakpointEvidenceSpark.LocalAssemblyHandler {
    ...
    private FermiLiteAssembly reviseAssembly( final FermiLiteAssembly assembly,
        ...
        for ( int id = 0; id != nContigs; ++id ) {
            if ( !representedContigs.contains(id) ) {
                contigList.add(assembly.getContig(id));
            }
        }
    ...
    private FermiLiteAssembly.Contig rcContig( final FermiLiteAssembly.Contig contig ) {
